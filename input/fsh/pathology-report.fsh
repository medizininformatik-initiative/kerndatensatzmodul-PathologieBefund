...
This profile is the first entry in a Bundle of `Bundle.type` "document"; the persistent unique `Bundle.identifier` should correspond to that of the Composition (Composition.identifier, see below), e.g. accession number including version number.  Optionally, further entries may be included. The Bundle contains a signature.
Only two sections, the Diagnostic Report (`patho-diagnostic-report`) as well as one or more additional complete reports, e.g. from a molecular genetic examination (`additional-diagnostic-report`), may be present in the Composition. The observation report sections described in the data set are represented in the [MII PR Patho Report](StructureDefinition-mii-pr-patho-report.html), a profile of the DiagnosticReport resource, as special grouper observations with text and atomic individual observations ([MII PR Patho Finding](StructureDefinition-mii-pr-patho-finding.html)).
"""
