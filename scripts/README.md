...
> **Vendored.** This directory is a copy of `scripts/` from
> [`mii-kds-module-template`](https://github.com/medizininformatik-initiative/mii-kds-module-template),
> taken when this module adopted the template's CI workflows. Fix things there,
> not here. `first-run-bootstrap.sh` (and its test) are deliberately absent: they
> set up a module *created from* the template, and this module predates it.
>
> Not every script below has a workflow here yet — the template's workflows are
> being adopted in steps, and the ones still missing are named in the table's
> "Run by" column.
